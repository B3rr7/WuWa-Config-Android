...
│  (9 screens) │◀────│  (~980 lines)  │◀────│  (per-backend)  │◀────│ ADB/Shizuku/  │◀────│ (game dir)  │
└─────────────┘     └───────────────┘     └────────────────┘     │ Root/SAF      │     └────────────┘
       │                     │                      │              └──────────────┘
       │                     │                      │
       ▼                     ▼                      ▼
...
7. [Data Model](#data-model-model)
8. [Services](#services)
9. [Dependencies](#dependencies-buildgradlekts)
10. [State Management](#state-management)
11. [File Storage](#file-storage)
12. [Preset Profiles Quick Reference](#preset-profiles-quick-reference)
13. [Color System](#color-system)
14. [Component Library](#component-library-uicomponentscomponentskt)
15. [Game Paths](#game-paths-gamepathskt)

---

## Navigation & Screens

Routes defined in `MainActivity.kt:117-191`. 9 screens via NavHost:
...
- **Custom Config** — file picker (`OpenMultipleDocuments`) for `.ini` files → review screen showing matched files + skipped → Apply button calls `viewModel.applyCustomFiles()`
...
- **Preset selector** — Potato / Performance / Balanced / High / Ultra
...
- Create backup (reads current device config → saves JSON internally + `.ini` files to `Downloads/WuWaConfig/Backups/`)
- Restore backup (pushes saved files back to device)
- Delete backup
- Auto-backup before custom config apply (configurable in settings)
...
- **`createBackup()`** — reads current device config → saves as JSON + `.ini` files to `Downloads/WuWaConfig/Backups/{name}/`
- **`restoreBackup()`** — reads backup files → pushes to device
...
4. Apply → ViewModel calls `ConfigManager.applyCustomConfigs()`
5. Backup created (if setting enabled)
6. Each file written to `cache/staging/`, pushed via backend, retried on failure
7. `KuroConfigMonitor.hash` refreshed with new MD5 + incremented `ModifyCount`
8. Log shows "SUCCESS: N file(s) applied (Engine.ini, ...)" and "Skipped (not provided): ..."
