...
object SmartBrain {
    ...
    fun scoreRecommendation(
        ...
        when (tier) {
            "flagship" -> {
                score += 30
                signals.add("Flagship GPU: +30")
            }
            "high" -> {
                score += 20
                signals.add("High-end GPU: +20")
            }
            "mid_high" -> {
                score += 10
                signals.add("Mid-high GPU: +10")
            }
            "mid" -> { /* +0 */ }
            "mid_low" -> {
                score -= 10
                signals.add("Low-mid GPU: -10")
            }
            else -> {
                score -= 20
                signals.add("Low-end GPU: -20")
            }
        }
        ...
        when {
            ram >= 8000 -> {
                score += 8
                signals.add("8GB+ RAM: +8")
            }
            ram >= 6000 -> {
                score += 5
                signals.add("6GB+ RAM: +5")
            }
            ram in 4000..5999 -> { /* +0 */ }
            ram in 1..3999 -> {
                score -= 15
                signals.add("<4GB RAM: -15")
            }
        }

        if (info.vulkanStatus == "available") {
            score += 8
        ...
        info.fpsActual?.let { actual ->
            info.fpsCap?.let { cap ->
                if (cap > 0) {
                    ...
                    when {
                        dropPct > 30 -> {
                            score -= 18
                            signals.add("FPS drop >30%: -18")
                        }
                        dropPct > 20 -> {
                            score -= 12
                            signals.add("FPS drop 20-30%: -12")
                        }
                        dropPct > 10 -> {
                            score -= 6
                            signals.add("FPS drop 10-20%: -6")
                        }
                        actual >= cap * 0.95f -> {
                            score += 5
                            signals.add("FPS at target: +5")
                        }
                    }
                }
            }
            if (actual < 30) {
                score -= 15
                signals.add("FPS <30: -15")
            }
            if (actual in 30f..44f) {
                score -= 8
                signals.add("FPS 30-45: -8")
            }
        }

        when {
            info.thermalEvents >= 5 -> {
                score -= 20
                signals.add("Thermal throttling x${info.thermalEvents}: -20")
            }
            info.thermalEvents >= 3 -> {
                score -= 12
                signals.add("Thermal events x${info.thermalEvents}: -12")
            }
            info.thermalEvents >= 1 -> {
                score -= 5
                signals.add("Thermal events x${info.thermalEvents}: -5")
            }
        }
        when {
            info.gpuOom >= 3 -> {
                score -= 30
                signals.add("GPU OOM x${info.gpuOom}: -30")
            }
            info.gpuOom >= 2 -> {
                score -= 20
                signals.add("GPU OOM x${info.gpuOom}: -20")
            }
            info.gpuOom >= 1 -> {
                score -= 12
                signals.add("GPU OOM x${info.gpuOom}: -12")
            }
        }
        when {
            info.dropFrames >= 15 -> {
                score -= 10
                signals.add("Frame drops x${info.dropFrames}: -10")
            }
            info.dropFrames >= 5 -> {
                score -= 5
                signals.add("Frame drops x${info.dropFrames}: -5")
            }
        }
        if (info.isLowMem == true) {
            score -= 15
            signals.add("Low memory device: -15")
        }
        if (info.forbiddenCvars > 0) {
            if (!allowRestrictedCvars) {
                score -= info.forbiddenCvars * 5
                signals.add("Forbidden CVars x${info.forbiddenCvars}: -5 each")
            }
        }
        when {
            info.networkErrors >= 10 -> {
                score -= 10
                signals.add("Network issues x${info.networkErrors}: -10")
            }
            info.networkErrors >= 5 -> {
                score -= 5
        ...
        info.screenPct?.let { sp ->
            when {
                ...
                sp < 70f -> {
                    score -= 10
                    signals.add("Low render scale <70%: -10")
                }
                else -> {}
            }
        }
        ...
        if (info.textureErrors > 0) {
            ...
            score -= texPenalty
            signals.add("Texture errors x${info.textureErrors}: -$texPenalty")
        ...
        if (cvars.isNotEmpty()) {
            ...
            val monitored = cvars.keys.count { cvarDatabase.isMonitored(it) }
            val differFromDefault =
                cvars.count { (k, v) ->
                    cvarDatabase.differsFromDefault(k, v)
                }
            if (unknown > 5) {
                score -= 5
                signals.add("$unknown unknown CVars in log: -5")
            }
            if (monitored > 10) {
                score += 3
                signals.add("$monitored monitored CVars tracked: +3")
            }
            if (differFromDefault > 20) {
                score += 5
                signals.add("$differFromDefault CVars differ from defaults (optimized): +5")
            }
            if (differFromDefault < 5 && cvars.size > 10) {
                score -= 8
                signals.add("Most CVars match game defaults (room to optimize): -8")
            }
        }
            ...
            if (shadowQ != null) {
                when {
                    shadowQ >= 3 && (tier == "mid_low" || tier == "low") -> {
                        score -= 6
                        signals.add("High shadows on low GPU: -6")
                    }
                    shadowQ >= 3 && ram < 6000 -> {
                        score -= 4
                        signals.add("High shadows + <6GB RAM: -4")
                    }
                }
            }
        ...
        if (info.forbiddenCvars > 0 && !allowRestrictedCvars) {
